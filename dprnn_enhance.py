"""
DPRNN enhanced version - setbecomekeyoptimization
based on current4.63 dBresults，targetimprovementto10+ dB

keyimprovement:
1. longerTraining + Cosine Annealinglearning rate
2. enhanceddataenhanced
3. multi-resolutionLoss Function
4. largermodel (optional)
...
class EnhancedMiniLibriMixDataset(Dataset):
    """enhanceddataset - strongerdataenhanced"""
    ...
    def __getitem__(self, idx):
        ...
        # ===== enhanceddataenhanced (onlyTrainingset) =====
        if self.split == 'train':
            # 1. stronger volume augmentation
            scale = np.random.uniform(0.5, 1.5)
            mixture = mixture * scale
            source1 = source1 * scale
            source2 = source2 * scale
            
            # 2. addsmallNoise (30%probability)
            ...
            if np.random.rand() < 0.5:
                shift = np.random.randint(-1600, 1600)  # ±0.2seconds @ 8kHz
...
# ==================== enhancedmainmodel ====================
class EnhancedDPRNN_TasNet(nn.Module):
    """enhanced versionDPRNN-TasNet"""
...
def multi_resolution_loss(est_sources, ref_sources, alpha=0.1):
    """
    multi-resolutionloss: SI-SNR + STFTloss
    
    Args:
        alpha: STFTlossweight
    """
    # time domainSI-SNRloss
    ...
    loss_time = torch.min(loss1, loss2).mean()
    
    # STFTdomainloss
    if alpha > 0:
        try:
            window = torch.hann_window(512).to(est_sources.device)
            
            def compute_stft_loss(est, ref):
                est_stft = torch.stft(est, n_fft=512, hop_length=128, 
                                     window=window, return_complex=True)
                ref_stft = torch.stft(ref, n_fft=512, hop_length=128,
                                     window=window, return_complex=True)
                return F.l1_loss(torch.abs(est_stft), torch.abs(ref_stft))
            
            stft_loss1 = compute_stft_loss(est_sources[:, 0], ref_sources[:, 0]) + \
                        compute_stft_loss(est_sources[:, 1], ref_sources[:, 1])
            
            stft_loss2 = compute_stft_loss(est_sources[:, 0], ref_sources[:, 1]) + \
                        compute_stft_loss(est_sources[:, 1], ref_sources[:, 0])
            
            loss_stft = torch.min(stft_loss1, stft_loss2)
            
            return loss_time + alpha * loss_stft
        except:
            # ifSTFTfailed，onlyusingtime domainloss
            return loss_time
    
    return loss_time


# ==================== Cosine Annealinglearning rateschedulingmodule ====================
class CosineAnnealingWarmupRestarts(torch.optim.lr_scheduler._LRScheduler):
    """withWarmupCosine Annealing"""
...
# ==================== Training Functions (support gradient accumulation) ====================
def train_epoch(model, dataloader, optimizer, device, gradient_accumulation_steps=1):
    ...
    for i, (mixture, sources) in enumerate(pbar):
        ...
        loss = multi_resolution_loss(est_sources, sources, alpha=0.1) / gradient_accumulation_steps
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for mixture, sources in tqdm(dataloader, desc='Evaluating'):
            ...
            est_sources = model(mixture)
            loss = multi_resolution_loss(est_sources, sources, alpha=0.1)
...
def main():
    config = {
        ...
        'batch_size': 2,        # reduce tosupportlargermodel
        'num_epochs': 200,      # increaseTrainingcount!
        'learning_rate': 5e-4,  # reduce initiallearning rate
        'L': 20,
        'N': 512,
        'H': 256,               # increase hidden layers!
        'R': 8,                 # increaseDPRNNnumber of layers!
        'C': 50,
        'dropout': 0.1,         # adddropoutprevent overfitting
        'warmup_epochs': 10,
    ...
    print("=" * 70)
    print("🚀 DPRNN enhanced versionTraining")
    print("=" * 70)
    print(f"\ndevice: {config['device']}")
    print(f"configuration: L={config['L']}, N={config['N']}, H={config['H']}, R={config['R']}, C={config['C']}")
    print(f"Training: {config['num_epochs']} epochs, LR={config['learning_rate']}, Dropout={config['dropout']}")
    ...
    for epoch in range(config['num_epochs']):
        ...
        print(f"\nTrainingSI-SNR: {train_sisnr:.4f} dB | ValidationSI-SNR: {val_sisnr:.4f} dB")
    ...
    plt.tight_layout()
    plt.savefig('training_enhanced.png', dpi=300)
    
    print(f"\n{'='*70}")
    print(f"Trainingcompleted!")
    print(f"bestSI-SNR: {-best_val_loss:.4f} dB")
    print(f"model: best_dprnn_enhanced.pth")
    print(f"{'='*70}")
